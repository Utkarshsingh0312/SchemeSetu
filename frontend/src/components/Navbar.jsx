...
import { Globe, Shield, LogOut, Menu, X, ArrowRight, Search } from 'lucide-react';
  ...
  return (
    <header 
      className={`sticky top-0 z-50 transition-all duration-300 border-b border-navy/10 font-sans ${
        scrolled 
          ? 'bg-[#FBF8F1]/95 backdrop-blur-md shadow-sm h-[70px]' 
          : 'bg-[#F3EEDF] h-[76px]'
      }`}
    >
      <div className="max-w-[1180px] mx-auto px-4 sm:px-7 h-full flex items-center justify-between gap-4">
        
        {/* BRAND & NAVIGATION LEFT GROUP */}
        <div className="flex items-center gap-[36px] lg:gap-[44px]">
          ...
          <Link to="/" className="flex items-center gap-3 group flex-none cursor-pointer">
            ...
            {/* BRAND NAME */}
            <div className="font-serif font-bold text-[24px] sm:text-[25px] text-navy tracking-tight group-hover:text-[#2C6350] transition-colors duration-200">
              Scheme<span className="text-marigold font-normal italic">Setu</span>
            </div>
          </Link>

          {/* DESKTOP NAVIGATION LINKS (QUIETER VISUAL ROLE) */}
          <nav className="hidden lg:flex items-center gap-[28px] font-sans text-[13.5px] font-medium text-navy">
            <a 
              href="/#why" 
        ...
        {/* DESKTOP CONTROLS & USER ACTIONS (RIGHT GROUP) */}
        <div className="hidden md:flex items-center gap-[24px] lg:gap-[32px] flex-none">
          
          {/* COMPACT LANGUAGE SWITCHER */}
          <button 
            onClick={toggleLanguage}
            className="h-[42px] px-3.5 rounded-[10px] border border-navy/15 bg-[#FBF8F1]/70 hover:bg-[#FBF8F1] hover:border-[#B7975A]/45 hover:-translate-y-[1px] text-navy transition-all flex items-center gap-2 cursor-pointer font-sans shadow-2xs"
          ...
          {user ? (
            <div className="flex items-center gap-[20px] lg:gap-[24px] font-sans text-xs">
              ...
              <Link 
                to="/passbook" 
                className="h-[46px] px-5 rounded-[12px] bg-[#FBF8F1] border border-navy/14 text-[#16213C] font-semibold text-[14px] hover:bg-[#16213C] hover:text-[#FBF8F1] hover:-translate-y-[1px] shadow-xs transition-all duration-200 flex items-center justify-center gap-2 cursor-pointer group whitespace-nowrap"
              >
